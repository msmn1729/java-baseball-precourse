...
### 플레이어의 입력
- [x] 3자리수를 입력한다.
- [x] 이 외의 입력이 들어왔을 때의 예외처리가 필요하다.

### 힌트
- [x] 자리와 숫자가 모두 일치하면 `스크라이트`로 정한다.
- [x] 숫자는 일치하나 자리가 다른 경우 `볼`로 정한다.
- [x] 같은 숫자가 하나도 없는 경우 `낫싱`으로 정한다.

### 게임 종료
- [x] 정답인 숫자와 3자리 숫자가 모두 일치할 때 게임을 종료한다.
- [x] 게임이 종료됐을 때 2가지를 선택할 수 있다.
  - [x] 재시작하려면 `1`을 입력한다.
  - [x] 게임을 종료하려면 `2`를 입력한다.  
  - [x] 이 외의 입력이 들어왔을 때의 예외처리가 필요하다.
  
### 예외 상황
- [x] 3자리수를 입력하지 않았을 경우 (ex. 1234)
- [x] 1~9 이외의 문자를 입력한 경우 (ex. a123)
- [x] 중복된 문자가 입력된 경우 (ex. 122)
